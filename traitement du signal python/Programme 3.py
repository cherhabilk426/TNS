...
while continuer_partie: # Tant qu'on doit continuer la partie
    # on demande Ã  l'utilisateur de saisir le nombre sur lequel il va miser
    nombre_mise = -1
    ...
    # A présent, on sélectionne la somme à  miser sur le nombre
    mise = 0
    while mise <= 0 or mise > argent:
        ...
        if mise > argent:
            print('Vous ne pouvez miser autant, vous n avez que {} $'. format(argent))

    # Le nombre misé et la mise ont été sélectionnés par l'utilisateur, on fait tourner la roulette
    numero_gagnant = randrange(50)
    print("La roulette tourne... ... et s'arrête sur le numéro {}". format(numero_gagnant))

    # On établit le gain du joueur
    if numero_gagnant == nombre_mise:
        print("Félicitations ! Vous obtenez {}$ !". format(mise * 3))
        argent += mise * 3
    elif numero_gagnant % 2 == nombre_mise % 2: # ils sont de la même couleur
        mise = ceil(mise * 0.5)
        print("Vous avez misé sur la bonne couleur. Vous obtenez {} $ !". format(mise))
        argent += mise
    else:
    ...
    if argent <= 0:
        ...
        print("Vous avez à  présent {} $". format(argent))
